...
var gameTimerId;
var gametime = 120; // game time in seconds
var gamescore = 0;
...
function dropoff()
{
    ...
    for(var house in HOUSES)
	{
		...
		{	
			console.log("We are in range of: " + house);
			for (var i = 0; i < shuttle.seats.length; i++)
			{
				if(shuttle.seats[i] != null && shuttle.seats[i].house == house)
				{
					found = true;
					console.log("found a dropoff for : " + shuttle.seats[i].name);
					$("#announcements").html($("#announcements").html() + "Dropped off: " + shuttle.seats[i].name + "<br>");
	...
	if(!found)
	{
		$("#announcements").html("We don't seem to be near a dropoff point!");
	}
}
...
function keystroke(event, state)
{
    ...
    else if (event.keyCode == 65 || event.keyCode == 97)
    {
        ...
        $("#announcements").html("Bus is movin'!");
        return false;
    }
...
function pickup()
{
    ...
    for (var i = 0; i < PASSENGERS.length; i++)
    {
    	...
    	var lon = PASSENGERS[i].placemark.getGeometry().getLongitude();
    	if(shuttle.distance(lat,lon) <= 15 && PASSENGERS[i]["passenger"] === false && HOUSES[PASSENGERS[i].house] != null)
    	{
	    	console.log("people within range: " + PASSENGERS[i].name);	
	    	found = true;
	    	
	    	if(!gamestarted)
    		{
    			$("#gameinstructions").html("");
    			gameTimerId = window.setInterval(function(){ updateTimer() }, 1000);
    			gamestarted = true;
    		}

	    	var seatnumber = getSeat();
	    	if(seatnumber != -1)
	    	{
				PASSENGERS[i]["passenger"] = true;
				score(1);
	    		shuttle.seats[seatnumber] = PASSENGERS[i];
	    		removeMarkers(PASSENGERS[i]);
	    		chart();
	    		$("#announcements").html($("#announcements").html() + "Picked up " + PASSENGERS[i].name + "<br>");
	    	}
	    	else
	    	{
    ...
    if(!found)
    {
    	$("#announcements").html("No one within range of bus for pickup!");
    }
}

//output current street location
function getloc()
{
...
// add a point to the score and display
function score(n)
{
	if(gamestarted === true)
	{
		gamescore += n;
		$("#score").html("Score: <strong>"+gamescore+"</strong>");
	}
}

// updates time counter and display
...
pads n number with width and optional padding char z
taken from stackoverflow -> http://stackoverflow.com/a/10073788
user: pointy -> http://stackoverflow.com/users/182668/pointy
...
function removeMarkers(passenger)
{
	console.log("Removing marker for : " + passenger.name);
